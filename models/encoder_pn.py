...
class LGA(nn.Module):
    def __init__(self, out_dim, alpha, beta, vv, ww):
        super().__init__()
        alpha, beta = 1, 1
        self.LoConv = LoConv(3, out_dim, alpha, beta, vv)

        self.HiConv = HiConv(3, out_dim, alpha, beta, ww)
    ...
    def forward(self, lc_xyz, lc_x, lc_rgb, knn_xyz, knn_x, knn_rgb):
        ...
        knn_x_w1 = self.LoConv(knn_xyz, knn_x, knn_rgb)
        knn_x_w2 = self.HiConv(lc_xyz, knn_xyz, knn_x, knn_rgb)

        return knn_x_w1, knn_x_w2
...
class HiConv(nn.Module):
    def __init__(self, in_dim, out_dim, alpha, beta, ww):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.alpha, self.beta = alpha, beta
        ...
        self.mlp = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=out_dim, kernel_size=1, bias=True),
        ...
        self.p = nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        self.ww = ww
    ...
    def forward(self, lc_xyz, knn_xyz, knn_x, knn_rgb):
        ...
        mean_knn_x_new = torch.mean(knn_x_new, dim=-1, keepdim=True) ## [B, C, N, K]
        relative_knn_x_new = knn_x_new - mean_knn_x_new

        h_xi_xj = self.mlp(knn_xyz)  # [B, d+3, npoint, k]                   
        W_xyz = torch.cos(h_xi_xj * 2 * torch.pi).cuda()   
        knn_x_new2 = relative_knn_x_new * W_xyz

        self.p.data = self.p.data.clamp(min=1, max=3)
        # knn_x_new = torch.pow(torch.abs(knn_x_new) + 1e-5, self.p)
        knn_x_new2 = torch.mul(torch.pow(torch.abs(knn_x_new2) + 1e-6, self.p), torch.sign(knn_x_new2))
        knn_x_new2 = self.bn(knn_x_new2)
        knn_x_new2 = self.activation(knn_x_new2)
        ...
        std_knn_x_new = torch.std(knn_x_new - mean_knn_x_new)
        knn_x_new = (knn_x_new - mean_knn_x_new) / (std_knn_x_new + 1e-6)
        knn_x_new1 = knn_x_new.permute(0, 3, 1, 2)

        return knn_x_new1 + knn_x_new2
    
# Parametric Encoder
class EncNP(nn.Module):  
    def __init__(self, input_points, num_stages, embed_dim, k_neighbors, alpha, beta, vv, ww):
        super().__init__()
        ...
        for i in range(self.num_stages):
            ...
            self.LGA_list.append(LGA(out_dim, self.alpha, self.beta, vv, ww))
    ...
    def forward(self, xyz, x, rgb, rgbx):
        ...
        for i in range(self.num_stages):
            ...
            # Local Geometry Aggregation
            knn_x_w1, knn_x_w2 = self.LGA_list[i](xyz, lc_x, rgb, knn_xyz, knn_x, knn_rgb)
            # Pooling
            x1 = self.Pooling_list[i](knn_x_w1)
            x2 = self.Pooling_list[i](knn_x_w2)
...
class Encoder_Seg(nn.Module):
    def __init__(self, input_points=2048, num_stages=5, embed_dim=144, k_neighbors=128, de_neighbors=6, alpha=1000, beta=50):
        ...
        self.EncNP_1 = EncNP(input_points, num_stages, embed_dim, k_neighbors, alpha, beta, vv, ww)
