...
class LGA(nn.Module):
    def __init__(self, out_dim, alpha, beta, vv, ww):
        super().__init__()
        alpha, beta = 1, 1
        self.LoConv = PosE_Geo(3, out_dim, alpha, beta, vv)
...
class HiConv(nn.Module):
    ...
    def forward(self, lc_xyz, knn_xyz, knn_x, knn_rgb):
        ...
        feat_dim = self.out_dim // (3 * 2)
        feat_range = torch.arange(feat_dim).float().cuda()     
        dim_embed = torch.pow(self.alpha, feat_range / feat_dim)

        knn_xyz_2 = knn_xyz.permute(0, 2, 3, 1)[..., None]
        div_xyz = torch.div(self.beta * knn_xyz_2.unsqueeze(-1), dim_embed) #* 0.1
        sin_xyz, cos_xyz = torch.sin(div_xyz), torch.cos(div_xyz)
        xyz_embed = torch.cat([sin_xyz, cos_xyz], dim=4).flatten(3)
        xyz_embed = xyz_embed.permute(0, 3, 1, 2)

        ###########
        W_2 = torch.cos(xyz_embed * 2 * torch.pi).cuda()  + torch.cos(xyz_embed * 2 * torch.pi + 0.5 * torch.pi).cuda()   
        knn_x_new2 = knn_x_new * W_2
        ###########
        knn_x_new = knn_x_new.permute(0, 2, 3, 1)
        ...
        knn_x_new = knn_x_new1 + knn_x_new2
...
class Encoder_Seg(nn.Module):
    def __init__(self, input_points=2048, num_stages=5, embed_dim=144, k_neighbors=128, de_neighbors=6, alpha=1000, beta=50):
        ...
        self.EncNP_2 = EncNP(input_points, num_stages, embed_dim, k_neighbors, alpha, beta, vv, ww)
